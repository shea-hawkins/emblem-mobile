...
- (BOOL)activateDataSet:(Vuforia::DataSet *)theDataSet
{
    ...
    if (success) {
        [self setExtendedTrackingForDataSet:_dataSetCurrent start:extendedTrackingEnabled];
    }
...
- (BOOL) menuProcess:(NSString *)itemName value:(BOOL)value
{
    ...
    if ([@"Flash" isEqualToString:itemName]) {
        ...
        bool result = [self setExtendedTrackingForDataSet:_dataSetCurrent start:value];
        if (result) {
